...
def main() -> None:
    ...
    reframe_code = f'''import bpy,json,os\nfrom bpy_extras.object_utils import world_to_camera_view\nfrom mathutils import Vector\nspec=json.loads({embedded!r})\nhero=bpy.data.objects.get(spec["hero_camera"]); target=bpy.data.objects.get(spec["target_empty"])\nif hero is None or hero.type!="CAMERA" or target is None: raise RuntimeError("Phase 8 cameras are missing")\nhero.location=spec["hero_location"]; target.location=spec["hero_target"]; hero.data.lens=spec["wide_lens"]\nif hero.location.y>=target.location.y-0.02 or hero.location.x>=target.location.x-0.02: raise RuntimeError("Hero camera is not on the checked southwest exterior diagonal")\nbpy.context.scene.camera=hero; bpy.context.view_layer.update()\nframing={{}}\nfor name,coordinate in spec["anchors"].items():\n    point=world_to_camera_view(bpy.context.scene,hero,Vector(coordinate)); framing[name]=[float(point.x),float(point.y),float(point.z)]\ninvalid=[name for name,p in framing.items() if not (0.02<=p[0]<=0.98 and 0.02<=p[1]<=0.98 and p[2]>0)]\nif invalid: raise RuntimeError("Wide hero framing lost anchors: "+repr(invalid))\nsubject_meshes=[obj for obj in bpy.data.objects if obj.type=="MESH" and obj.get("material","")!="terrain"]\nsubject_points=[world_to_camera_view(bpy.context.scene,hero,obj.matrix_world @ Vector(corner)) for obj in subject_meshes for corner in obj.bound_box]\nsubject_front=[point for point in subject_points if point.z>0]\nif len(subject_front)!=len(subject_points): raise RuntimeError("Southwest subject extends behind the hero camera")\nsubject_extent=[min(point.x for point in subject_front),min(point.y for point in subject_front),max(point.x for point in subject_front),max(point.y for point in subject_front)]\nif not (0.0<=subject_extent[0] and -0.12<=subject_extent[1] and subject_extent[2]<=1.0 and subject_extent[3]<=1.0): raise RuntimeError("Southwest subject crop exceeds checked limits: "+repr(subject_extent))\nbpy.context.scene["aec_camera_composition"]=spec["composition"]\nbpy.ops.wm.save_as_mainfile(filepath={str(output_scene)!r},compress=True)\nprint("PORTABLE_CAMERA_WIDE_DATA="+json.dumps({{"composition":spec["composition"],"location":list(hero.location),"target":list(target.location),"lens":hero.data.lens,"resolution":[bpy.context.scene.render.resolution_x,bpy.context.scene.render.resolution_y],"anchors":framing,"subject_extent":subject_extent}},separators=(",",":"),sort_keys=True))\n'''
